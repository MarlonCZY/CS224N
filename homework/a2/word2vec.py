...
def naiveSoftmaxLossAndGradient(
    ...
    # 1. probability of a word w being the context word of the center word, size: (number of word in the vocab, 1)
    probs = softmax(np.dot(outsideVectors, centerWordVec))[:, np.newaxis]
    # 2.  loss function, a float number
    loss = float(-np.log(probs[outsideWordIdx]))
    # 3. gradient of center vector, size: (word vector length, )
    gradCenterVec = -outsideVectors[outsideWordIdx] + np.sum(probs * outsideVectors, axis=0)
    # 4. gradient of outside vectors size: (number of word in the vocab, word vector length)
    deltaY = np.copy(probs)
    deltaY[outsideWordIdx] -= 1
    gradOutsideVecs = deltaY * centerWordVec
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    currentOutsideWordIdx = [word2Ind[word] for word in outsideWords]
    currentCenterWordVec = centerWordVectors[currentCenterWordIdx]

    for outsideIdx in currentOutsideWordIdx:
        currentLoss, currentGradCenterVec, currentGradOutsideVecs = \
            word2vecLossAndGradient(currentCenterWordVec, outsideIdx, outsideVectors, dataset)
        loss += currentLoss
        gradCenterVecs[currentCenterWordIdx] += currentGradCenterVec
